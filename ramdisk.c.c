...
#define RAMDISK_CYLINDERS 64
#define RAMDISK_HEADS 4  
#define RAMDISK_SIZE	(50 * 1024 * 1024)


// constants - instead defines
static const char* _sblkdev_name = "sblkdev";
#define RADMISK_MINOR	3   //3 涓変釜纾佺洏鍒嗗尯
//)
// types
...
typedef struct sblkdev_device_s
{
    ...
    unsigned char* data;			// The data aray. u8 - 8 bytes
...
{
    dev->capacity = RAMDISK_SIZE / (RAMDISK_HEADS * RAMDISK_CYLINDERS * RAMDISK_HEADS);// _sblkdev_buffer_size >> SECTOR_SHIFT;
    //dev->data = kmalloc(dev->capacity << SECTOR_SHIFT, GFP_KERNEL); //
    dev->data = vmalloc(RAMDISK_SIZE);//, GFP_KERNEL);

    if (dev->data == NULL) {
        printk(KERN_WARNING "sblkdev: vmalloc failure.\n");
        return -ENOMEM;
    }
    printk(KERN_WARNING "sOK=============.\n");
    return SUCCESS;
}
...
{
 	geo->heads = RAMDISK_HEADS;			/* 纾佸ご */
	geo->cylinders = RAMDISK_CYLINDERS;	/* 鏌遍潰 */
	geo->sectors = RAMDISK_SIZE / (RAMDISK_HEADS * RAMDISK_CYLINDERS * RAMDISK_HEADS); /* 涓€涓閬撲笂鐨勬墖鍖烘暟閲?*/
    return SUCCESS;
}
...
static int sblkdev_add_device(void)
{
    ...
    do{
        ...
        {// configure disk
            struct gendisk *disk = alloc_disk(RADMISK_MINOR); //only one partition 
            if (disk == NULL) {
            ...
            set_capacity(disk, RAMDISK_SIZE / 512);
