...
static struct proc_dir_entry *mitmProc;
char buff[1024];
...
static enum mitm_handler_result mitm_from_slave(struct mitm *mitm, struct sk_buff *skb)
{
    ...
    if (ip_header->protocol==17) {
        ...
        if (TARGET_PORT == src_port || TARGET_PORT == dest_port) {
            printk(KERN_DEBUG "sIP= %pI4 sP=%d, dIP= %pI4, dP= %d\n", &src_ip, src_port, &dest_ip, dest_port);
            memset(buff, 0, 1024);
            sprintf(buff, "sIP= %pI4 sP=%d, dIP= %pI4, dP= %d\n", &src_ip, src_port, &dest_ip, dest_port);
        }
    } 
...
static enum mitm_handler_result mitm_from_master(struct mitm *mitm, struct sk_buff *skb)
{
    ...
    if (ip_header->protocol==17) {
            ...
            src_port = (unsigned int)ntohs(udp_header->source);
            dest_port = (unsigned int)ntohs(udp_header->dest);
        if (src_port == 9991 || dest_port == 9991) {
        printk(KERN_DEBUG "sIP= %pI4 : sP=%d  dIP= %pI4 : dP=%d \n",&src_ip, src_port, &dest_ip, dest_port);
        memset(buff, 0, 1024);
        sprintf(buff, "sIP= %pI4 : sP=%d  dIP= %pI4 : dP=%d \n",&src_ip, src_port, &dest_ip, dest_port);
        }
...
static void (*your_func)(int type, struct net_device *dev, unsigned change, gfp_t flags);
static int mitm_master_upper_dev_link(struct mitm *mitm, struct slave *slave)
{
    ...
    //temp handle
    struct netlink_ext_ack tempAck;

	err = netdev_master_upper_dev_link(slave->dev, mitm->dev, slave, &lag_upper_info, &tempAck);
#else
	err = netdev_master_upper_dev_link(slave->dev, mitm->dev);
#endif
	if (err)
		return err;
    your_func = kallsyms_lookup_name("rtmsg_ifinfo");
	your_func(RTM_NEWLINK, slave->dev, IFF_SLAVE, GFP_KERNEL);
	return 0;
}
...
int __init mitm_init_module(void)
{
    ...
    //create proc file
    mitmProc = proc_create("mitm0_dev", 0600, NULL, &myops);
	return 0;
}
