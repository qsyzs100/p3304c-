...
#define MIB_SEC(mib) (mib * 1024 * 1024 / 512)
...
static PartTable def_part_table =
{
    ...
    },
    {
        ...
        end_head: 0x0,
        end_cyl: 0x0,
        abs_start_sec: MIB_SEC(30),
        sec_in_part: MIB_SEC(20)
    },
    {},
    {}
};
...
{
    {
        ...
        },
        {
            ...
            end_sec: 0x00,
            end_cyl: 0x00,
            abs_start_sec: MIB_SEC(10),
            sec_in_part: MIB_SEC(10)
        },
    },
    {
        {
...
static int rb_open(struct block_device *bdev, fmode_t mode)
{
    ...
    printk(KERN_INFO "myramdisk: Inode number is %d\n", unit);
...
{
    printk(KERN_INFO "myramdisk: Device is closed\n");
}

static void copy_mbr(u8 *disk)
{
    memset(disk, 0x0, MBR_SIZE);
    *(unsigned long *)(disk + MBR_DISK_SIGNATURE_OFFSET) = 0x78D2314D;
    memcpy(disk + PARTITION_TABLE_OFFSET, &def_part_table, PARTITION_TABLE_SIZE);
    *(unsigned short *)(disk + MBR_SIGNATURE_OFFSET) = 0xAA55;
}
static void copy_br(u8 *disk, int abs_start_sector, const PartTable *part_table)
{
    u8 *diskT = (disk + abs_start_sector * SECTOR_SIZE);
    memset(diskT, 0x0, BR_SIZE);
    memcpy(diskT + PARTITION_TABLE_OFFSET, part_table,
        PARTITION_TABLE_SIZE);
    *(unsigned short *)(diskT + BR_SIGNATURE_OFFSET) = BR_SIGNATURE; 
}
...
static int rb_transfer(struct request *req)
{
    ...
    rq_for_each_segment(bv, req, iter)
    {
        ...
        {
            printk(KERN_ERR "myramdisk: Should never happen: "
        ...
        printk(KERN_DEBUG "myramdisk: Sector Offset: %lld; Buffer: %p; Length: %d sectors\n",
    ...
    {
        printk(KERN_ERR "myramdisk: bio info doesn't match with the request info");
        ret = -EIO;
    }
...
static int __init rb_init(void)
{
    ...
    copy_mbr(dev_data);
    copy_br(dev_data, MIB_SEC(30), &def_log_part_table[0]);
    copy_br(dev_data, MIB_SEC(40), &def_log_part_table[1]);

    rb_dev.size = ret;
 
    /* Get Registered */
    rb_major = register_blkdev(rb_major, "myramdisk");
    if (rb_major <= 0)
    {
    ...
    if (rb_dev.rb_queue == NULL)
    {
        ...
        unregister_blkdev(rb_major, "myramdisk");
        ram_device_free();
        return -ENOMEM;
    }
    ...
    sprintf(rb_dev.rb_disk->disk_name, "myramdisk");
    ...
    printk(KERN_INFO "myramdisk: Ram Block driver initialised (%d sectors; %d bytes)\n",
