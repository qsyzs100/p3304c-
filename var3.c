...
static int major = 206;  
static int minor = 0;
static struct class *cls;
static struct device *var3_device;
int g_count = 0;
int g_flag = 0;
...
static int __init proc_var3_init(void)
{	
	int ret = 0;
	...
	ret = register_chrdev(major, "var3", &fops);

	cls = class_create(THIS_MODULE, "var3");
    if(IS_ERR(cls))
    {
        unregister_chrdev(major, "var3");
        return -EBUSY;
    }
    var3_device = device_create(cls, NULL, devno, NULL, "var3");
    if(IS_ERR(var3_device))
    {
        class_destroy(cls);
        unregister_chrdev(major,"var3");
        return -EBUSY;
...
static void __exit proc_var3_exit(void)
{
	proc_remove(entry);
...
module_init(proc_var3_init);
module_exit(proc_var3_exit);
